...
function genSheet(){
    ...
    for(let column = 0; column < 27; column++){
        lists[0][column] = {text: alphabets[column], id: column, style:{}};
    }
    for(let row = 1; row < 101; row++){
        lists[row][0] = {text: row, id: row*100};
        for(let column = 1; column < 27; column++){
            lists[row][column] = {text: "", id: row*100+column, style:{}};
        }
    }
    return(lists);
}
...
function FakeSheet(){
    ...
    function change_val(x, y, val){
        let new_sheet = [...sheets];
        new_sheet[x][y].text = val;
        SetSheet(new_sheet);
    }
    ...
    function reinit_id(sheet){
        ...
        for(let column = 1; column < sheet_size[1]; column++){
            new_sheet[0][column] = {text: alphabets[parseInt(column/27)]+alphabets[(column-1)%26+1], id: column, style:{}};
        }
    ...
    function add_row(){
        ...
        for(let column = 0; column < sheet_size[0]; column++){
            new_sheets[column].splice(mouse_pos[1], 0, {text: "", id: sheet_size[0]*100+sheet_size[1]*100+column, style:{}});
        }
    ...
    function add_column(){
        ...
        for(let row = 0; row < sheet_size[1]; row++){
            new_column[row] = {text: "", id: sheet_size[0]*100+sheet_size[1]*100+row, style:{}};
        }
    ...
    return(
        <>
            ...
            <div className="sheet">
                <ARbtn add={add_row}/ > <DRbtn delete_row={delete_row}/ >
                <ACbtn add={add_column}/ > <DCbtn delete_column={delete_column}/>
                <table className="table-style" border="2">
                    <tbody>
                        {sheets.map(item => (<tr>{item.map(i => <Cell text={i.text} id={i.id} change={change_val} darken={focus_darken} style = {i.style}/>)}</tr>))} 
                    </tbody> 
                </table>
            </div>
        </>
